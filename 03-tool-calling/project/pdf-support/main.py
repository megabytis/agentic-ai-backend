...
def encode_pdf(pdf_path):
    """Converting pdf to base64 string"""
    with open(pdf_path, "rb") as pdf_file:
        return base64.b64encode(pdf_file.read()).decode("utf-8")


def create_pdf_message(pdf_path, prompt_text):
    """Creating a message with pdf and text"""
...
def handle_pdf_command(pdf_path, custom_prompt=None):
    """Handle PDF upload/analysis in chat"""

    default_prompt = "Summarize this PDF document in detail. What are the key points?"
...
def start_chatbot():
    ...
    while True:
        ...
        if user_input.lower() == "/bye":
            ...
            # Format: /pdf path/to/pdf.jpg [optional]
            parts = user_input.split(" ", 2)
            pdf_path = parts[1]
            custom_prompt = parts[2] if len(parts) > 2 else None

            try:
                handle_pdf_command(pdf_path=pdf_path, custom_prompt=custom_prompt)
            except Exception as e:
